...
func = @(x)PEATbT_fitting(x,Pertur,Data,FitRange);
...
for g = 1:length(Deg)
    Hand_real(g,:) = PEATbT(FitParam,Pertur(g,:),size(Pertur,2));
end
...
for g = 1:length(deg)
    ...
    hand_real = PEATbT(FitParam,pertur(g,:),80);
...
%%
function RMSE = PEATbT_fitting(param,Pertur,Data,FitRange)
...
for Group = 1:size(Data,1)
    ...
    Index = FitRange(1);
    Index2 = FitRange(2);
    Hand_real = PEATbT(param,pertur,length(data));
...
function Hand_real = PEATbT(param,pertur,trialNum)
a = param(1);
b = param(2);
sig_h = param(3);
sig_g = param(4);
