...
for g = 1:length(deg)
    ...
    [hand_real,hand_est] = PEATbTProp(fitParam,pertur(g,:),Tn);
...
for r = 1:length(Ratios)
    ...
    for g = 1:length(deg)
        angle = deg(g);
        pertur(g,:) = ones(1,Tn)*angle;
        [~,hand_est] = PEATbTProp(fitParam,pertur(g,:),Tn);
...
%%
function [Hand_real,Hand_est] = PEATbTProp(param,pertur,trialNum)
...
[c_mean2,~] = CueCombine(c_mean,0,0,c_sig_sq,sig_h,0,1);
Hand_est2(t) = c_mean2;
 Hand_est(t) = c_mean;
    if t<trialNum
